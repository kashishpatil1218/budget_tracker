...
class UserDbHelper {
  ...
  Future<void> insertData(String name, email, phone, age) async {
  ...
  Future<void> upadteData(String name, email, phone, age, Uint8List img) async {
    Database? db = await database;
    String sql =
        "UPDATE user SET name=?,email=?, phone=?,age=?,img=? WHERE id=1";
    List args = [name, email, phone, age, img];
    await db!.rawUpdate(sql, args);
  }

  Future<List<Map<String, Object?>>> fetchUserData() async {
    Database? db = await database;
